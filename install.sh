...
url="vless://$myuid@$serverIp:$port?type=http&security=reality&encryption=none&pbk=$pub&fp=chrome&path=$path&sni=$sni&sid=$shortId#$name"
...
newJson=$(echo "$json" | jq \
    ...
    '.inbounds[0].port= '"$(expr "$port")"' |
     ...
     .inbounds[0].settings.clients[0].id = $myuid |
